Class Github extends AHK.Lang.Class.Meta.Call{
	static github_API:="https://api.github.com/"
	static github:="https://github.com/"
	Call(self,Call:="")	{
		static cache:=new Ahk.Hash(),requestor:= ComObjCreate("WinHttp.WinHttpRequest.5.1"),API_Token:=Ahk.config(this,"API_Token")
		...
		if(cvalue != "")	
			return cvalue
		if(API_Token == "" and !Ahk.config(this,"SkipToken",,0))	{	;Checks for API Authentication token
			...
			if(Errorlevel)	{
				...
				ifmsgbox, no
					Ahk.config(this,"SkipToken",1)
			}
			else
				API_Token:=Outputvar,Ahk.config(this,"API_Token",API_Token)
		}
		...
		if(requestor.status == 200)		{	;Validate API response
			response:=Ahk.obj.Json.Load(requestor.ResponseText)	;Convert API Response to object
	...
	Info(Keys*)	{
		...
		Loop % Keys.Length()
		{
			if(subcall != "")	{
				if(instr(subcall,"{"))
					subcall:=Ahk.String.Substr.Strip.byChar(subcall,"{","}",Keys[A_Index])
				if(!instr(subcall,"{"))
	...
	Class Repository extends Ahk.GitHub{
		__New(Repository_Ref,Repository_Author:="",Repository_Name:="")		{
			if(Repository_Author != "" and Repository_Name != "")
				...
				if(Instr(Repository_Ref,this.github) == 1)	;Github URL is provided
					this.info_id:="repos/" Ahk.String.Substr.Delimited(Repository_Ref,"/",3,2)
				if(Instr(Repository_Ref,this.github_API) == 1)	;Github API URL is provided
					this.info_id:=Ahk.String.Substr.Delimited(Repository_Ref,"/",3,3)
			}
		}
	}
	Class Author extends Ahk.GitHub{
		__New(Author_Ref){
				if(instr(Repository_Ref,"users/") == 1)	;Info_ID is provided
					this.info_id:=Repository_Ref
				if(Instr(Repository_Ref,this.github) == 1)	;Github URL is provided
					this.info_id:="users/" Ahk.String.Substr.Delimited(Repository_Ref,"/",3,1)
				if(Instr(Repository_Ref,this.github_API) == 1)	;Github API URL is provided
					this.info_id:=Ahk.String.Substr.Delimited(Repository_Ref,"/",3,2)
		}
	}
	Class Commit extends Ahk.GitHub{
		__New(Commit_Ref,Repository_Author:="",Repository_Name:="",Commit_Sha:="")
		{
			if(Repository_Author != "" and Repository_Name != "" and Commit_Sha != "")
				...
				if(Instr(Commit_Ref,this.github) == 1)	;Github URL is provided
					this.info_id:="repos/" Ahk.String.Substr.Delimited(Commit_Ref,"/",3,2) "/commits/" Ahk.String.Substr.Delimited(Commit_Ref,"/",6,1)
				if(Instr(Commit_Ref,this.github_API) == 1)	;Github API URL is provided
					this.info_id:=Ahk.String.Substr.Delimited(Commit_Ref,"/",3,5)
			}
