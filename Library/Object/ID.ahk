	Class ID extends lib.obj.meta.call_functor{
		/*
			Gives Objects an ID.
		...
		*/
		Call(self, byref Obj,mode:=0)	{
			...
			if(!isobject(Obj[OID_Ref]) or Lib.Obj.Class.Tree_Name(Obj[OID_Ref]) != "Lib.Obj.ID.generic_ID") ;Checks if it still doesn't have a generic_ID
				Lib.tshoot("Error - Unable to set Object ID",Obj)	;Throws Error because it can't set object ID
		...
		Class meta_ID{
				...
				*/
				__ID(mode:=0) 	{
					...
					return !block?block:instr(mode[1],"__ID")==1?0:!check_serializing?-1:Lib.obj.Serialize.isSerializing()!=this.__ID(-2) ;Determines whether action should be blocked. -1 = Doesn't care. 0=Allow Write to object 1=Block Write to object
				}
				__Set(Key*) {
					if(this.__ID(Key) == 1)
						return
				...
				__Call(funct,Key*)	{
					static bypass:=0
					if(funct == "__Delete")
						Lib.obj.ID.retired(this.__ID(1))
