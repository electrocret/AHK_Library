 ...
 */
	...
	*/
	class JSON	{
		Read(FileName,line:=1,Default_Value:="")	{
			FileRead, File_Content, %FileName%
			return File_Content == ""? Default_Value:this.Load(StrSplit(File_Content,"`n")[line])
		}
		Write(FileName,wObject,line:=1) {
			FileRead, File_Content, %FileName%
			File_Content:=StrSplit(File_Content,"`n")
			File_Content[line]:=this.Dump(wObject)
