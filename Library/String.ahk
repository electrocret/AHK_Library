Class String{
	...
	Class Index extends Lib.Obj.Meta.Call_Construct{
	...
	Instr(Haystack,Needle,CaseSensititve:=false,StartingPos:=1,Occurrence:=1)	{
		Index:=new Lib.String.Index("",Needle,Haystack,CaseSensititve,StartingPos,Occurrence)
		return Index.Index != 0?Index:""
	}
	Class Substr extends Lib.Obj.Meta.Call_Functor{
		...
		Delimited(String,delimiter:=",",Start_delim:=1,Value_count:=1,StartingPos:=1)		{
			return this.trim(String,Lib.String.InStr(String,delimiter,true,StartingPos,Start_Delim),Lib.String.InStr(String,delimiter,true,StartingPos,Start_Delim + Value_count))
		}
		Class Strip extends Lib.Obj.Meta.Call_Functor{
			...
			bychar(byref String, Start_Char,End_Char,Replace:="",Strip_All:=0,byref Relative_String:="")	{
				s_start:=Lib.String.inStr(String,Start_Char)
				if(s_start.index != 0)	{
					s_end:=Lib.String.inStr(String,End_Char,,s_start)
					if(Relative_String != "")
